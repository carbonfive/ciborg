username = ENV['SUDO_USER'].strip

directory "#{node["hubot"]["home"]}/.ssh" do
  mode 0700
  owner "hubot"
end

execute "copy id_rsa" do
  destination_path = "#{node["hubot"]["home"]}/.ssh/id_rsa"
  source_path = "/home/#{username}/.ssh/id_rsa"
  files = "#{source_path} #{destination_path}"
  command "cp #{files}"
  only_if { (::File.exists?(source_path)) && !system("diff -q #{files}") }
end

file "#{node["hubot"]["home"]}/.ssh/id_rsa" do
  mode 0600
  owner "hubot"
end

file "#{node["hubot"]["home"]}/.ssh/known_hosts" do
  mode 0600
  owner "hubot"
end

execute "add github to known hosts if necessary" do
  github_string = 'github.com,207.97.227.239 ssh-rsa '
  command "echo '#{github_string}' >> #{node["hubot"]["home"]}/.ssh/known_hosts"
  not_if "grep '#{github_string}' #{node["hubot"]["home"]}/.ssh/known_hosts"
end
